...
static size_t max_work_items_size;
...
void cl_build_program()
{
	...
	const char *source_str = load_kernel_source();
	...
	clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(max_work_items_size),
					&max_work_items_size, NULL);
}

void cl_init(char *kernel_name, size_t state_length, size_t roundkeys_length)
{
	if (program == nullptr)
	{
		cl_build_program();
...
void cl_encrypt(uint8_t *text, uint32_t *roundkeys)
{
	size_t global = state_length / 16;

	//printf("Info: using global work-item size: %u\n", global);

	/*
	if (global > max_work_items_size)
	{
		global = max_work_items_size;
	}
	*/
