...
PUBLIC	?err@@3HA					; err
PUBLIC	?cp_platform@@3PAU_cl_platform_id@@A		; cp_platform
PUBLIC	?device_id@@3PAU_cl_device_id@@A		; device_id
PUBLIC	?context@@3PAU_cl_context@@A			; context
PUBLIC	?queue@@3PAU_cl_command_queue@@A		; queue
PUBLIC	?program@@3PAU_cl_program@@A			; program
PUBLIC	?kernel@@3PAU_cl_kernel@@A			; kernel
PUBLIC	?state@@3PAU_cl_mem@@A				; state
PUBLIC	?roundkeys@@3PAU_cl_mem@@A			; roundkeys
PUBLIC	?argi@@3IA					; argi
PUBLIC	?keys@@3PAEA					; keys
PUBLIC	?iv@@3PAEA					; iv
PUBLIC	?start@@3JA					; start
PUBLIC	?end@@3JA					; end
...
PUBLIC	?count@@3IA					; count
PUBLIC	?inbuf@@3PAEA					; inbuf
PUBLIC	?outbuf@@3PAEA					; outbuf
_BSS	SEGMENT
?err@@3HA DD	01H DUP (?)				; err
?cp_platform@@3PAU_cl_platform_id@@A DD 01H DUP (?)	; cp_platform
?device_id@@3PAU_cl_device_id@@A DD 01H DUP (?)		; device_id
?context@@3PAU_cl_context@@A DD 01H DUP (?)		; context
?queue@@3PAU_cl_command_queue@@A DD 01H DUP (?)		; queue
?program@@3PAU_cl_program@@A DD 01H DUP (?)		; program
?kernel@@3PAU_cl_kernel@@A DD 01H DUP (?)		; kernel
?state@@3PAU_cl_mem@@A DD 01H DUP (?)			; state
?roundkeys@@3PAU_cl_mem@@A DD 01H DUP (?)		; roundkeys
?argi@@3IA DD	01H DUP (?)				; argi
?keys@@3PAEA DD	01H DUP (?)				; keys
?iv@@3PAEA DD	01H DUP (?)				; iv
?start@@3JA DD	01H DUP (?)				; start
?end@@3JA DD	01H DUP (?)				; end
?inbuf@@3PAEA DD 01H DUP (?)				; inbuf
?outbuf@@3PAEA DD 01H DUP (?)				; outbuf
_BSS	ENDS
_DATA	SEGMENT
?count@@3IA DD	080H					; count
_DATA	ENDS
...
CONST	ENDS
PUBLIC	_main
EXTRN	?cl_encrypt_test@@YAXXZ:PROC			; cl_encrypt_test
EXTRN	?key_schedule_test@@YAXXZ:PROC			; key_schedule_test
...
; 49   : 	#ifdef TEST_KEY_SCHEDULE
; 50   : 		key_schedule_test();
...
; 53   : 
; 54   : 	/*
; 55   : 	char *source = "./aes-encrypt.cl";
; 56   : 
; 57   : 	printf("Loading \"%s\"\n", source);
; 58   : 	start = clock();
; 59   : 	init(source);
; 60   : 	create_kernel("aes_encrypt_256");
; 61   : 	end = clock();
; 62   : 
; 63   : 	printf("Ready (%ums)\n", end - start);
; 64   : 
; 65   : 	printf("\nGPU info:\n");
; 66   : 	unsigned long int param_value;
; 67   : 	
; 68   : 	param_value = clGetDeviceInfo(device_id, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(param_value), &param_value, NULL);
; 69   : 	{
; 70   : 		double memory = (double) param_value;
; 71   : 		char units[][4] = {"B  ", "KiB", "MiB", "GiB"};
; 72   : 		char *unit = units[0];
; 73   : 		for (int i = 1; memory >= 1024; i++)
; 74   : 		{
; 75   : 			memory /= 1024;
; 76   : 			unit = units[i];
; 77   : 		}
; 78   : 		printf("  local memory size        %.2f %s\n", memory, unit);
; 79   : 	}
; 80   : 	clGetDeviceInfo(device_id, CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(param_value), &param_value, NULL);
; 81   : 	{
; 82   : 		double memory = (double) param_value;
; 83   : 		char units[][4] = {"B  ", "KiB", "MiB", "GiB"};
; 84   : 		char *unit = units[0];
; 85   : 		for (int i = 1; memory >= 1024; i++)
; 86   : 		{
; 87   : 			memory /= 1024;
; 88   : 			unit = units[i];
; 89   : 		}
; 90   : 		printf("  global memory size       %.2f %s\n", memory, unit);
; 91   : 	}
; 92   : 	clGetDeviceInfo(device_id, CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(param_value), &param_value, NULL);
; 93   : 	printf("  maximum compute units    %u\n", param_value);
; 94   : 	clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(param_value), &param_value, NULL);
; 95   : 	printf("  maximum work-group size  %u\n", param_value);
; 96   : 	clGetDeviceInfo(device_id, CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(param_value), &param_value, NULL);
; 97   : 	printf("  maximum work-item sizes  %u\n", param_value);
; 98   : 
; 99   : 	inbuf = new unsigned char[BUFFER_SIZE];
; 100  : 	outbuf = new unsigned char[BUFFER_SIZE];
; 101  : 	int inlen = BUFFER_SIZE,
; 102  : 		outlen = 0;
; 103  : 
; 104  : 	if (argc > 1)
; 105  : 	{
...
; 107  : 	}
; 108  : 
; 109  : 	// fill input buffer
; 110  : 	memset(inbuf, 0, sizeof(*inbuf) * BUFFER_SIZE);
; 111  : 
; 112  : 	// simple key and iv
; 113  : 	keys = new unsigned char[ROUNDKEYS_SIZE];
; 114  : 	iv = new unsigned char[IV_SIZE];
; 115  : 	unsigned int key_length = (ROUNDKEYS_SIZE == 240) ? 32 : ((ROUNDKEYS_SIZE == 208) ? 24 : 16);
; 116  : 
; 117  : 	for (unsigned int i = 0; i < key_length; i++) keys[i] = i + 1;
; 118  : 
; 119  : 	for (unsigned int i = 0; i < IV_SIZE; i++) iv[i] = 0;
; 120  : 
; 121  : 	key_schedule(keys, ROUNDKEYS_SIZE / 16);
; 122  : 
; 123  : 	printf("\nKey info:\n");
; 124  : 	printf("  key length: %u bits\n", key_length);
; 125  : 	printf("  expanded key length: %u bytes\n", ROUNDKEYS_SIZE);
; 126  : 	printf("  rounds: %u\n", ROUNDKEYS_SIZE / 16);
; 127  : 
; 128  : 	create_buffers();
; 129  : 
; 130  : 	clEnqueueWriteBuffer(queue, state,     CL_TRUE, 0, sizeof(*inbuf) * BUFFER_SIZE,   inbuf,  0, NULL, NULL);
; 131  : 	clEnqueueWriteBuffer(queue, roundkeys, CL_TRUE, 0, sizeof(*keys) * ROUNDKEYS_SIZE, keys,   0, NULL, NULL);
; 132  : 
; 133  : 	start = clock();
; 134  : 	execute(count);
; 135  : 	end = clock();
; 136  : 
; 137  : 	err = clEnqueueReadBuffer(queue, state, CL_TRUE, 0, sizeof(*inbuf) * BUFFER_SIZE, outbuf, 0, NULL, NULL);
; 138  : 
; 139  : 	if (err != CL_SUCCESS) {
; 140  : 		printf("Error: failed to read from buffer\n", err);
; 141  : 		exit(EXIT_FAILURE);
; 142  : 	}
; 143  : 
; 144  : 	for (uint32_t i = 0; i < count; i++)
; 145  : 	{
; 146  : 		printf("%2x ", outbuf[i]);
; 147  : 		if ((i+1) % 16 == 0)
; 148  : 		{
; 149  : 			printf("\n");
; 150  : 		}
