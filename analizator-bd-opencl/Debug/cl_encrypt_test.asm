...
_TEXT	SEGMENT
tv73 = -620						; size = 4
_rk$6718 = -420						; size = 240
_data$6701 = -172					; size = 4
_i$6697 = -160						; size = 4
...
; 199  : 
; 200  : 	for (unsigned int i = 0; i < test_data_size; i++)

	mov	DWORD PTR _i$6697[ebp], 0
	jmp	SHORT $LN15@cl_encrypt
$LN14@cl_encrypt:
	mov	eax, DWORD PTR _i$6697[ebp]
	add	eax, 1
	mov	DWORD PTR _i$6697[ebp], eax
$LN15@cl_encrypt:
	mov	eax, DWORD PTR _i$6697[ebp]
...
; 202  : 		Encrypt_Data &data = test_data[i];
	...
	mov	DWORD PTR _data$6701[ebp], eax
...
; 204  : 		if (last_kernel == 0 || last_kernel != data.length)
	...
	mov	eax, DWORD PTR _data$6701[ebp]
...
; 234  : 		memcpy(rk, data.key, data.length);
	...
	mov	ecx, DWORD PTR [eax]
	push	ecx
	mov	edx, DWORD PTR _data$6701[ebp]
	add	edx, 4
	push	edx
...
; 240  : 		cl_encrypt(rk, data.length, result, DATA_LENGTH);
	...
	push	eax
	mov	ecx, DWORD PTR _data$6701[ebp]
...
; 243  : 		{
; 244  : 			printf("  #%u (%u bit) pass\n", i+1, data.length);

	mov	eax, DWORD PTR _data$6701[ebp]
	mov	ecx, DWORD PTR [eax]
	push	ecx
	mov	edx, DWORD PTR _i$6697[ebp]
	add	edx, 1
	push	edx
