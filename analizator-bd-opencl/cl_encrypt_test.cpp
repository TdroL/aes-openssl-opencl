...
void cl_encrypt_test()
{
	...
	for (unsigned int i = 0; test_data[i].length; i++)
	{
		...
		{
		case 16:
			cl_init("aes_encrypt_128", DATA_LENGTH, 
					RKLENGTH(data.length*8)*sizeof(uint32_t));
			break;
		case 24:
			cl_init("aes_encrypt_192", DATA_LENGTH, 
					RKLENGTH(data.length*8)*sizeof(uint32_t));
			break;
		case 32:
			cl_init("aes_encrypt_256", DATA_LENGTH, 
					RKLENGTH(data.length*8)*sizeof(uint32_t));
			break;
		}
		...
		rijndaelSetupEncrypt((unsigned long *) rk, data.key, data.length*8);

		//memcpy(result, data.plaintext, DATA_LENGTH);

		cl_encrypt(data.plaintext, rk);
		//local_encrypt(plaintext, rk);

		if (memcmp(data.plaintext, data.ciphertext, DATA_LENGTH) == 0)
		{
			printf("  #%02u (%u bytes) pass\n", i+1, data.length);
		}
		else
		{
			printf("  #%02u (%u bytes) fail\n", i+1, data.length);
			/*
...
{
	int bits[3] = {16, 24, 32};
	uint8_t key[RKLENGTH(256)*4];
	uint8_t text[DATA_LENGTH];
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 2; j++)
		{
			for (int k = 0; k < 3; k++)
			{
				...
				{
				case 0:
					memset(text, 0x00, DATA_LENGTH);
					break;
				case 1:
					for (int l = 0; l < DATA_LENGTH; text[l] = l, l++);
					break;
				}
				...
				{
				case 0:
					memset(key, 0x00, bits[i]);
					break;
				case 1:
					memset(key, 0xff, bits[i]);
					break;
				case 2:
					for (int l = 0; l < bits[i]; key[l] = l, l++);
					break;
				}

				printf("{\n\t%u,\n\t{", bits[i]);

				for (int l = 0; l < bits[i]; l++)
				{
					if (l % 8 == 0)
					{
						printf("\n\t\t");
				...
				for (int l = 0; l < DATA_LENGTH; l++)
				{
					...
					printf("0x%02x, ", text[l]);
				}

				printf("\n\t},\n\t{");

				int nrounds = rijndaelSetupEncrypt((unsigned long *) key, key, bits[i]*8);

				for (int l = 0; l < DATA_LENGTH; l += 16)
				{
					rijndaelEncrypt((unsigned long *) key, nrounds, text+l, text+l);
				}
